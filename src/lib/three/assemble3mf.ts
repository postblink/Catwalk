// Main-thread side of the worker-offloaded 3MF path: turns the plain
// typed-array geometry the worker produced (see parse3mfCore.ts) back into a
// THREE.Group of meshes. This is the only step that touches THREE — it's cheap
// (a few BufferAttribute wraps, no parsing), so it stays on the main thread.
 ...
 * surface material so geometry-only 3MF matches the STL/OBJ look.
 *
 ...
 */
export function assemble3mf(meshes: MeshBuffers[]): THREE.Group {
